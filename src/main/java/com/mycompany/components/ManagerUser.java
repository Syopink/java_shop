...
public class ManagerUser extends javax.swing.JPanel {
    ...
    private List<rowsUser> rowsList = new ArrayList<>(); // Danh sách các hàng người dùng
    private Customer customer;
    private boolean showingDeleted = false; // Trạng thái hiện tại 
    ...
    public ManagerUser() {
        initComponents();
        initCustomerList(); // Tải dữ liệu người dùng khi khởi tạo
    ...
    private void loadCustomers(List<Customer> customers) {
        ...
        for (Customer customer : customers) {
            rowsUser row = new rowsUser(index, customer, new customers()); // Truyền số thứ tự vào rowsUser
    ...
    private void searchCustomers() throws SQLException {
        ...
        // Kiểm tra trạng thái đang hiển thị người dùng đã xóa hay chưa
        boolean isDeleted = showingDeleted;  // Trạng thái xóa hay chưa xóa

        if (name.isEmpty() && email.isEmpty()) {
            // Nếu không nhập gì, hiển thị toàn bộ danh sách người dùng theo trạng thái xóa
            List<Customer> allCustomers = cs.searchUser(name, email, isDeleted); // Tìm kiếm với isDeleted
            loadCustomers(allCustomers); // Hiển thị toàn bộ danh sách
            return;
        }

        // Tìm kiếm người dùng với tên, email và trạng thái xóa
        List<Customer> filteredList = cs.searchUser(name, email, isDeleted); // Tìm kiếm với isDeleted
        if (filteredList.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Không tìm thấy người dùng nào phù hợp!");
        } else {
    ...
    private void initComponents() {
        ...
        rowsUser = new com.mycompany.components.util.rowsUser();
        jScrollPane2 = new javax.swing.JScrollPane();
        rowsUser2 = new com.mycompany.components.util.rowsUser();
    ...
    private void jbtnDelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnDelActionPerformed
        // TODO add your handling code here:
        // Tạo danh sách các người dùng cần xóa
        List<Customer> usersToDelete = new ArrayList<>();
        customers cs = new customers();  // Khởi tạo đối tượng customers

        // Kiểm tra tất cả các hàng trong rowsList và tìm các người dùng đã chọn
        for (rowsUser row : rowsList) {
            if (row.isSelected()) {  // Kiểm tra xem row có được chọn không
                usersToDelete.add(row.getCustomer());  // Lấy người dùng từ row
            }
        }
        ...
        if (!usersToDelete.isEmpty()) {
            ...
            try {
                // Xóa người dùng trong cơ sở dữ liệu thông qua DAL
                cs.deleteCustomers(usersToDelete);
                JOptionPane.showMessageDialog(this, "Đã xóa thành công người dùng.");
                initCustomerList();  // Làm mới danh sách người dùng
            } catch (SQLException e) {
                JOptionPane.showMessageDialog(this, "Lỗi khi xóa người dùng: " + e.getMessage());
            }
        } else {
            JOptionPane.showMessageDialog(this, "Không có người dùng nào được chọn.");
        }
    ...
    private void jDelMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jDelMouseClicked
        ...
        try {
            ...
            if (showingDeleted) {
                // Hiển thị người dùng đã xóa (isDeleted = '1')
                userList = cs.getAllUsersDeleted(); // Lấy danh sách người dùng đã xóa

                // Kiểm tra nếu không có người dùng đã xóa
                if (userList.isEmpty()) {
                    JOptionPane.showMessageDialog(this, "Không có người dùng đã xóa!");
                    showingDeleted = false;  // Đặt lại trạng thái về chưa xóa
                    userList = cs.getAllUsers();  // Lấy lại danh sách người dùng chưa xóa
                } else {
                    jLabel28.setText("DANH SÁCH NGƯỜI DÙNG ĐÃ XÓA");
                    jbtnDel.setVisible(active);      // Ẩn nút xóa
                    jRestore.setVisible(!active);    // Hiển thị nút phục hồi
                }
            } else {
                // Hiển thị người dùng chưa xóa (isDeleted = '0')
                userList = cs.getAllUsers(); // Lấy danh sách người dùng chưa xóa
                jLabel28.setText("DANH SÁCH NGƯỜI DÙNG");
            ...
            loadCustomers(userList);

        } catch (SQLException e) {
            JOptionPane.showMessageDialog(this, "Lỗi khi tải danh sách người dùng: " + e.getMessage());
        }
    }//GEN-LAST:event_jDelMouseClicked
    private List<Customer> getSelectedUser() {
        List<Customer> selectedCustomers = new ArrayList<>();

        // Kiểm tra tất cả các hàng trong rowsList và tìm các người dùng đã được chọn
        for (rowsUser row : rowsList) {
            if (row.isSelected()) {  // Kiểm tra xem row có được chọn không
                selectedCustomers.add(row.getCustomer());  // Lấy người dùng từ row
            }
        }
    ...
    private void jRestoreMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jRestoreMouseClicked
        // TODO add your handling code here:
        try {
            // Lấy danh sách các người dùng đã chọn (có thể sử dụng phương thức isSelected() trong các hàng người dùng)
            List<Customer> usersToRestore = getSelectedUser();  // Hàm này cần lấy danh sách người dùng đã được chọn
            List<Customer> usersList = new ArrayList<>();

            // Kiểm tra nếu có người dùng được chọn để phục hồi
            if (usersToRestore.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Vui lòng chọn ít nhất một người dùng để phục hồi.");
                return;
            }

            // Thực hiện phục hồi người dùng (set isDeleted = 0)
            boolean isRestored = cs.restoreCustomers(usersToRestore);  // Phương thức restoreCustomers bạn phải định nghĩa

            if (isRestored) {
                JOptionPane.showMessageDialog(this, "Người dùng đã được phục hồi thành công!");

                // Chuyển lại về danh sách người dùng chưa xóa
                showingDeleted = false;  // Đặt trạng thái về chưa xóa

                // Cập nhật lại giao diện và hiển thị danh sách người dùng chưa xóa
                usersList = cs.getAllUsers();  // Lấy lại danh sách người dùng chưa xóa
                loadCustomers(usersList);  // Tải lại danh sách lên giao diện
                jLabel28.setText("DANH SÁCH NGƯỜI DÙNG");

                jbtnDel.setVisible(true);  // Hiển thị nút xóa
                jRestore.setVisible(false);  // Ẩn nút phục hồi
            } else {
                JOptionPane.showMessageDialog(this, "Có lỗi xảy ra khi phục hồi người dùng.");
            }

        } catch (SQLException e) {
            JOptionPane.showMessageDialog(this, "Lỗi khi phục hồi người dùng: " + e.getMessage());
        }
 ...
 // Duyệt qua từng dòng trong danh sách rowsList và đảo ngược trạng thái chọn của nó
    ...
    private void updateCustomerList() {
        try {
            // Kiểm tra trạng thái xem có đang hiển thị người dùng đã xóa hay không
            List<Customer> customerList;

            if (showingDeleted) {
                // Nếu đang hiển thị người dùng đã xóa, không cần thay đổi
                customerList = cs.getAllCustomersDeleted(); // Lấy danh sách người dùng đã xóa
            } else {
                // Nếu không, lấy danh sách người dùng chưa xóa
                customerList = cs.getAllCustomers(); // Lấy danh sách người dùng chưa xóa
            }

            // Cập nhật lại giao diện với danh sách người dùng
            loadCustomers(customerList);  // Gọi phương thức để tải lại danh sách lên giao diện

            // Nếu đang hiển thị người dùng đã xóa, thay đổi tiêu đề và các nút
            if (showingDeleted) {
            ...
            JOptionPane.showMessageDialog(this, "Lỗi khi cập nhật danh sách người dùng: " + e.getMessage());
        }
    }
    ...
    private com.mycompany.components.util.rowsUser rowsUser;
    private com.mycompany.components.util.rowsUser rowsUser2;
